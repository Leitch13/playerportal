...
// dashboards, wearing the "Cyan Shell" treatment that matches the live
// app: cyan gradient chrome (top bar / sidebar) around a dark navy
// workspace (#080e18 bg, #0f1a2b cards, #1d2c42 hairlines, #4ecde6
// accent). Interiors stay intentionally DARK — product screenshots on a
// light marketing page. These can be swapped for real screenshots once
// captured.
// ════════════════════════════════════════════════════════════════════

const SHELL_GRADIENT = 'linear-gradient(170deg,#12a2bd,#0b7f96)'

// Thin cyan shell strip for the compact bento mocks — the full chrome bar
// lives on ParentHubMock (the hero device).
function ShellStrip() {
  return <div className="h-1.5 w-full" style={{ background: SHELL_GRADIENT }} />
}

export function ParentHubMock() {
  return (
    <div className="rounded-2xl border border-black/40 bg-[#080e18] overflow-hidden shadow-2xl shadow-[#021e26]/60">
      {/* Cyan shell chrome */}
      <div className="flex items-center gap-1.5 px-4 py-3" style={{ background: SHELL_GRADIENT }}>
        <div className="w-2.5 h-2.5 rounded-full bg-white/30" />
        <div className="w-2.5 h-2.5 rounded-full bg-white/30" />
        <div className="w-2.5 h-2.5 rounded-full bg-white/30" />
        <div className="ml-4 text-[11px] text-white/80 font-mono">theplayerportal.net / dashboard</div>
      </div>

      {/* Content — dark navy workspace */}
      <div className="p-5 space-y-4">
        ...
        {/* Next class card */}
        <div className="rounded-xl border border-[#1d2c42] bg-[#0f1a2b] p-4">
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button className="text-[11px] px-3 py-1.5 rounded-full border border-[#1d2c42] text-white/70">Details</button>
          </div>
        </div>
...
function MiniPlayerCard({ name, initial, active, muted }: { name: string; initial: string; active?: boolean; muted?: boolean }) {
  return (
    <div className={`rounded-xl border p-3 flex items-center gap-3 ${muted ? 'border-dashed border-[#1d2c42] bg-transparent' : 'border-[#1d2c42] bg-[#0f1a2b]'}`}>
    ...
    <div className="rounded-2xl border border-black/40 bg-[#080e18] overflow-hidden">
      ...
      <div className="p-5 space-y-3">
        ...
        <div className="space-y-1.5">
          {players.map((p) => (
            <div key={p.name} className="flex items-center gap-2 rounded-lg bg-[#0f1a2b] border border-[#1d2c42] px-3 py-2">
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="space-y-1.5">
          {['Colleen O\'Neil', 'Jenna Bruce', 'Lauren Wisely', 'Ritchie Hutchison'].map((n) => (
            <div key={n} className="flex items-center gap-2 rounded-lg bg-[#0f1a2b] border border-[#1d2c42] px-3 py-2">
...
function MigStat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className={`rounded-xl border p-3 ${accent ? 'border-[#4ecde6]/30 bg-[#4ecde6]/5' : 'border-[#1d2c42] bg-[#0f1a2b]'}`}>
  ...
  return (
    <div className="rounded-2xl border border-black/40 bg-[#080e18] overflow-hidden">
      ...
      <div className="p-5 space-y-3">
        ...
        <div className="rounded-xl bg-[#0f1a2b] border border-[#1d2c42] p-3">
      ...
      <div className="p-5 space-y-3">
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={m.from} className="rounded-xl bg-[#0f1a2b] border border-[#1d2c42] p-3">
