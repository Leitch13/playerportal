...
export async function POST(request: NextRequest) {
  ...
  // Migration Safety Phase 1 (ce65a25) removed sendInvitations from body —
  // invitations are now sent in an explicit second step after admin reviews
  // conflicts. The body type no longer carries that field.
  ...
  const summary: ImportSummary = {
    imported: 0,
    skipped: 0,
    // Migration Safety Phase 1 + 1.1 expanded summary shape: cross-academy
    // conflicts (P0 protection), DOB-mismatch warnings, errors, and the
    // invitations[] list the wizard sends in a second explicit step after
    // admin reviews the import results.
    conflicts: [],
    warnings: [],
    errors: [],
    invitations: [],
