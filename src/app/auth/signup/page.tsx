        ...
        <div className="rounded-2xl border border-[#1e1e1e] bg-[#141414] p-4 sm:p-8 shadow-2xl">
          ...
          {step === 1 && (!showAcademySearch || orgName) && (
            <form onSubmit={handleCreateAccount} className="space-y-3 sm:space-y-4">
              {/* Google Sign-In — Phase 1. Visible only when the academy slug
                  has resolved (orgName set) AND the feature flag allows this
                  slug. Hidden otherwise — there's no Google flow without an
                  org context per migration 094's trigger contract. */}
              {orgName && orgSlug && isSocialLoginEnabled(orgSlug) && !isStaffInvite && (
                <>
                  <button
                    type="button"
                    onClick={async () => {
                      setError('')
                      setLoading(true)
                      const r = await signInWithGoogle({ orgSlug, next: '/dashboard' })
                      if (!r.ok) {
                        setError(r.error)
                        setLoading(false)
                      }
                    ...
                    disabled={loading}
                    className="w-full flex items-center justify-center gap-2.5 py-2.5 sm:py-3 rounded-full bg-white text-black text-sm font-semibold hover:bg-white/90 transition-colors disabled:opacity-60"
                  >
